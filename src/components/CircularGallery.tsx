...
function createTextTexture(gl: OGLRenderingContext, text: string, font: string, color: string) {
  ...
  const fontSizePx = sizeMatch ? parseFloat(sizeMatch[1]) : 30;
  ...
  onResize({
    ...
    if (viewport) {
      ...
      if ((this.plane.program.uniforms as any).uViewportSizes) {
        (this.plane.program.uniforms as any).uViewportSizes.value = [this.viewport.width, this.viewport.height];
      }
    }
...
export const CircularGallery: React.FC<CircularGalleryProps> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cx('w-full h-full overflow-hidden cursor-grab active:cursor-grabbing', fontClassName, className)}
      {...props}
    />
  );
};
